...
</div>`})}const y=class y{transform(e){return e&&typeof e=="string"?Dt(e):""}};y.ɵfac=function(t){return new(t||y)},y.ɵpipe=$({name:"admonitionTransform",type:y,pure:!0});let B=y;const v=class v{transform(e){if(!e)return"";const t=new Map;let o=e;const r=/:::footnotes\s*\n([\s\S]*?)\n:::/g,g=r.exec(e);if(g){const m=g[1],p=/^\[\^([^\]]+)\]:\s*(.+)$/gm;let M;for(;(M=p.exec(m))!==null;){const f=M[1].trim(),j=M[2].trim();f&&t.set(f,j)}o=e.replace(r,"").trim()}else{const p=/\n\n(\[\^[^\]]+\]:.+\n?)+$/.exec(e);if(p){const M=p[0],f=/^\[\^([^\]]+)\]:\s*(.+)$/gm;let j;for(;(j=f.exec(M))!==null;){const Y=j[1].trim(),q=j[2].trim();Y&&t.set(Y,q)}o=e.substring(0,e.length-M.length).trim()}}const u=new Map,I=new Map;let b=0;const N=/\[\^([^\]]+)\]/g;if(o.replace(N,(m,p)=>{const M=String(p).trim();return u.has(M)||(b+=1,u.set(M,b),I.set(b,M)),""}),o=o.replace(N,(m,p)=>{const M=String(p).trim(),f=u.get(M);return f?`<sup><a data-fn="fn-${f}" id="fnref-${f}" class="footnote-ref" role="button" tabindex="0">[${f}]</a></sup>`:m}),u.size>0){let m=`
